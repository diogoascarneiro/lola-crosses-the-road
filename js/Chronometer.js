...
function printMinutes() {
 let minutes = currentGame.timer.computeTwoDigitNumber(currentGame.timer.getMinutes());
 minDecElement.textContent = minutes[0];
 minUniElement.textContent = minutes[1];
}

function printSeconds() {
 let seconds = currentGame.timer.computeTwoDigitNumber(currentGame.timer.getSeconds());
 secDecElement.textContent = seconds[0];
 secUniElement.textContent = seconds[1];
}

function printMilliseconds() {
  let millisec = currentGame.timer.computeTwoDigitNumber(currentGame.timer.getMilliseconds());
